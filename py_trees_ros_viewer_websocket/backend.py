...
class SnapshotStream(object):
    ...
    def _connect_on_init(self, timeout_sec=1.0):
        ...
        console.logdebug("rclpy.spin_until_future_complete")
        response = future.result()
        self.topic_name = response.topic_name
        # connect to a snapshot stream
        start_time = time.monotonic()
        while True:
            console.logdebug("searching for topic publishers [{}]".format(self.topic_name))
            ...
            if self.node.count_publishers(self.topic_name) > 0:
                console.logdebug("found publishers [{}]".format(self.topic_name))
                break
            time.sleep(0.1)
        ...
        self.tree_publisher = self.node.create_publisher(
            msg_type=String,
            topic='/tree_js_parsed_data',
            qos_profile=utilities.qos_profile_latched()
        )
        console.logdebug("  ...ok [backend]")

    def publish_tree(self, tree_string: str):
        msg = String()
        msg.data = str(tree_string)
    ...
    def create_service_client(self, key: str):
        ...
        Raises:
            ...
            :class:`~py_trees_ros.exceptions.TimedOutError`: if it times out waiting for the server
        """
        console.loginfo('create_service_client {}'.format(key))
...
class Backend(qt_core.QObject):
    ...
    def __init__(self, parameters):
        ...
        self.already_knew_node_names = []
        self.desired_node_name = 'tree'
        self.is_snapshot_stream_connected = False
    ...
    def discover_namespaces(self):
        ...
        new_node_names = self.node.get_node_names()
        console.logdebug("new_node_names {}[backend]".format(new_node_names))
        ...
        console.logdebug("discovered {}[backend]".format(self.discovered_namespaces))
        console.logdebug("new_namespaces {}[backend]".format(new_namespaces))
        if self.already_knew_node_names != new_node_names and len(new_namespaces) > 0:
            self.already_knew_node_names = new_node_names
            ...
            console.logdebug("snapshot_stream {}[backend]".format(self.snapshot_stream))
            has_node_name = self.desired_node_name in new_node_names
            console.logdebug("has_node_name {}[backend]".format(has_node_name))
            selected_namespace = new_namespaces[0]
            self.connect(selected_namespace, not has_node_name)
        self.discovered_timestamp = time.monotonic()

    def connect(self, namespace, should_disconnect):
        """
        ...
        """
        if self.is_snapshot_stream_connected and should_disconnect:
            self.is_snapshot_stream_connected = False
            console.logdebug("cancelling existing snapshot stream connection [][backend]")
            self.snapshot_stream.shutdown()
            self.snapshot_stream = None

        if not self.is_snapshot_stream_connected and not should_disconnect:
            console.logdebug("creating a new snapshot stream connection [{}][backend]".format(namespace))
            self.is_snapshot_stream_connected = True
    ...
    def tree_snapshot_handler(self, msg: py_trees_msgs.BehaviourTree):
        ...
        """
        console.logdebug("2 handling incoming tree snapshot [backend]")
        colours = {
            "Sequence": "#FFA500",
            "Selector": "#00FFFF",
            "Parallel": "#FFFF00",
        ...
        for blackboard_variable in msg.blackboard_on_visited_path:
            blackboard_variables[blackboard_variable.key] = utilities.normalise_name_strings(blackboard_variable.value)
        for behaviour in msg.behaviours:
            ...
            tree["behaviours"][behaviour_id] = {
                ...
                "details": utilities.normalise_name_strings(behaviour.additional_detail),
                "children": [str(conversions.msg_to_uuid4(child_id)) for child_id in behaviour.child_ids],
                "data": {
                    "Class": utilities.normalise_name_strings(behaviour.class_name),
                    "Feedback": utilities.normalise_name_strings(behaviour.message),
                },
            }
            if behaviour.blackboard_access:
                variables = []
                for variable in behaviour.blackboard_access:
                    variables.append(utilities.normalise_name_strings(variable.key + " ({})".format(variable.value)))
                    tree["blackboard"]["behaviours"].setdefault(behaviour_id, {})[variable.key] = utilities.normalise_name_strings(variable.value)
        ...
        if self.snapshot_stream.parameters.blackboard_activity:
            ...
            for item in msg.blackboard_activity:
                ...
                xhtml_snippet += (
                    ...
                    "<td style=[doubleQuote]text-align: center;[doubleQuote]>" + xhtml.yellow + item.activity_type + xhtml.reset + "</td>"
                    "<td style=[doubleQuote]text-align: center;[doubleQuote]>" + xhtml.normal + item.client_name + xhtml.reset + "</td>"
                    "<td>" + info + "</td>"
                    "</tr>"
                )
            xhtml_snippet += "</table>"
            str_xhtml_snippet = str(xhtml_snippet)
            skipped_str = str_xhtml_snippet.replace('"', '\"')
            tree["activity"] = [skipped_str]

        javascript_command = "{}".format(tree).replace("'", '"')
        self.snapshot_stream.publish_tree(javascript_command)
        console.logdebug("javascript_command")
        # console.logdebug(javascript_command)
